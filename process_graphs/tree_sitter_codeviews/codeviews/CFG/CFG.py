...
class CFGGraph:
    ...
    def to_networkx(self, CFG_node_list, CFG_edge_list):
        ...
        for node in CFG_node_list:
            ...
            G.add_node(node[0], label = label, type_label = node[3], node_type=node[3], method = node[4], code = node[5])
        for edge in CFG_edge_list:
            G.add_edge(edge[0], edge[1], controlflow_type = edge[2], edge_type = edge[2], label = edge[2], color = 'red')
        return G
